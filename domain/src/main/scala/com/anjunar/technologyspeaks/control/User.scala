...
class User extends Identity with OwnerProvider with SecurityUser with EntityContext[User] {
  ...
  @OneToOne(cascade = Array(CascadeType.ALL), orphanRemoval = true, targetEntity = classOf[UserInfo], mappedBy = "user")
  @PropertyDescriptor(title = "Info", naming = true)
  var info: UserInfo = uninitialized

  @OneToOne(cascade = Array(CascadeType.ALL), orphanRemoval = true, targetEntity = classOf[Address], mappedBy = "user")
  ...
  val schema = new EntitySchemaDef[User](classOf[User]) {
    ...
    val emails = column[util.Set[EMail]]("emails")
      ...
      .withDynamicLinks((user, ctx, session) => {
        User.findViewByUser(user)(using session)
          .thenCompose(view => ManagedProperty.managedView(session, view, "emails"))
      })
    val info = column[UserInfo]("info", views = Set("full", "application", "form", "table"))
      ...
      .withDynamicLinks((user, ctx, session) => {
        User.findViewByUser(user)(using session)
          .thenCompose(view => ManagedProperty.managedView(session, view, "info"))
      })
    val address = column[Address]("address", views = Set("full", "form"))
      ...
      .withDynamicLinks((user, ctx, session) => {
        User.findViewByUser(user)(using session)
          .thenCompose(view => ManagedProperty.managedView(session, view, "address"))
      })
  }

...
class JsonObjectParamReader extends ParamReader {
  ...
  override def read(ctx: RoutingContext, sessionHandler: SessionHandler, javaType: ResolvedClass, annotations: Array[Annotation], state: StateDef[?], factory : Stage.Session): CompletionStage[Any] = CompletableFuture.completedFuture(ctx.body().asJsonObject())
